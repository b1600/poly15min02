...
from ..pricing.fees import taker_fee_per_share
...
def kelly_headroom(settings, price: float = 0.5) -> float:
    """Ratio of the *smallest* order Kelly can ask for to the largest the
    caps allow, at `price`. <= 1.0 means Kelly sizing is live; > 1.0 means
    it is structurally inert -- every order is clipped by a cap, so
    `bankroll` and `kelly_fraction` have no effect on behaviour at all.

    This is a real failure mode, not a hypothetical: the entry gate and
    the sizer are set independently, and it is easy to require an edge so
    large that the implied Kelly stake always exceeds the position caps.
    The 2026-09-03 sweep hit exactly that -- 36 combinations, of which the
    entire `kelly_fraction` axis was a no-op, because 100% of orders were
    clipped to a flat `paper_trade_size`.
    """
    from ..pricing.fees import taker_fee_per_share

    if not 0.0 < price < 1.0:
        raise ValueError("price must be in (0, 1)")
    # cheapest edge that still clears the gate (uncertainty buffer -> 0,
    # i.e. the most favourable case for a small stake)
    min_raw_edge = (
        settings.min_edge_to_trade
        + taker_fee_per_share(price, settings)
        + price * (settings.slippage_buffer_bps / 10000.0)
    )
    f_min = min_raw_edge / (1.0 - price)
    smallest_kelly_shares = settings.bankroll * settings.kelly_fraction * f_min / price
    largest_allowed_shares = min(
        settings.paper_trade_size,
        settings.max_inventory_imbalance,  # clips the first order too, from an empty book
        settings.max_notional_per_market / price,
    )
    return smallest_kelly_shares / largest_allowed_shares
...
class PositionManager:
    ...
    def _check_matched_arb(
        ...
        # both legs are taken, and the fee is price-dependent per leg --
        # a matched pair pays more when both legs sit near 50c than when
        # the pair is lopsided, even at identical combined cost
        fees = taker_fee_per_share(vwap_up, s) + taker_fee_per_share(vwap_down, s)
        net_lock_in = 1.0 - combined - fees
