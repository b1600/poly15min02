...
def main() -> None:
    ...
    # realized_pnl is already net of fees (PaperExecutor bakes fees into each
    # position's cost_basis before computing pnl at resolution) -- fees_paid
    # is tracked separately only for visibility, so gross PnL is derived by
    # adding it back rather than realized_pnl needing any further deduction.
    print(f"\nWindows replayed: {result.num_windows_replayed}")
    print(f"Gross PnL:        {result.realized_pnl + result.fees_paid:+.2f}")
    ...
    # kill_switch_active reflects only whether the switch is still tripped
    # at the very end of the replay -- BacktestEngine resets it once per
    # simulated UTC day (see engine.py), so trigger_count is the more
    # useful signal for how often the daily loss limit actually got hit
    # across the whole dataset.
    print(f"Kill switch trips (daily loss limit breached): {result.kill_switch_trigger_count}")
    print(f"Kill switch active at end of replay: {result.kill_switch_active}")
