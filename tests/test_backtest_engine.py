...
from poly15m.backtest.engine import BacktestEngine, run_backtest
...
def test_backtest_resets_kill_switch_on_new_utc_day(tmp_path):
    """A live kill switch trip never clears itself (see risk/limits.py),
    but BacktestEngine deliberately resets it once per simulated UTC day
    -- otherwise one bad early day would silently zero out every later
    day's evaluation for the rest of a multi-day replay/sweep."""
    db_path = tmp_path / "fixture.db"
    day1_open = datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc).timestamp()
    day2_open = datetime(2024, 1, 2, 10, 0, 0, tzinfo=timezone.utc).timestamp()
    c1, *_ = build_fixture_db(db_path, condition_id="condA", open_ts=day1_open, trend_per_sec=0.05)
    c2, *_ = build_fixture_db(db_path, condition_id="condB", open_ts=day2_open, trend_per_sec=0.05)

    settings = make_settings()
    engine = BacktestEngine(settings)
    engine.risk_gate.trigger_kill_switch("pre-tripped for test")
    assert engine.risk_gate.kill_switch_active

    result = engine.run(db_path, [c1, c2])

    fills_by_market = dict(
        engine.db._conn.execute("SELECT condition_id, COUNT(*) FROM fills GROUP BY condition_id").fetchall()
    )
    assert fills_by_market.get("condA", 0) == 0  # kill switch blocked all of day 1
    assert fills_by_market.get("condB", 0) > 0  # day-2 rollover reset it, trading resumed

    assert result.kill_switch_trigger_count == 1
