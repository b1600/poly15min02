...
def test_matched_arb_triggers_when_combined_ask_under_one():
    ...
    # 60 shares a side at 0.45 would be $54 of combined notional, over the
    # $20 per-market cap, so the pair is scaled down to fit it
    assert up_intent.size == down_intent.size
    combined_notional = up_intent.size * 0.45 + down_intent.size * 0.45
    assert combined_notional == pytest.approx(SETTINGS.max_notional_per_market)
...
def test_directional_kelly_sizes_via_kelly_formula():
    ...
    assert intent.limit_price == 0.5
    # f* = (0.7-0.5)/(1-0.5) = 0.4
    # notional = bankroll * kelly_fraction * f* = 1000 * 0.05 * 0.4 = $20
    # size = 20 / 0.5 = 40 shares -- under paper_trade_size (60) and the
    # imbalance cap (60), so Kelly is what actually determines the size.
    # This is the point of the parameter: if a cap wins here instead,
    # kelly_fraction has stopped doing anything (see kelly_headroom).
    expected = SETTINGS.bankroll * SETTINGS.kelly_fraction * 0.4 / 0.5
    assert intent.size == pytest.approx(expected)
