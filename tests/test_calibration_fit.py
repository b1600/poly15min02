...
def build_calibration_db(tmp_path, n, true_prob_fn, seed=0, name="cal.db"):
    ...
    for i, dev in enumerate(deviations):
        ...
        # t_remaining is fixed so log(t) is constant across rows: the
        # time-varying model then collapses to a plain slope on deviation,
        # which is what these tests are about.
        db.insert_fair_value_log(
            condition_id, float(i), None, None, 450.0, None, float(dev),
...
def test_load_calibration_dataset_excludes_unresolved_and_null_deviation(tmp_path):
    ...
    db.insert_fair_value_log("resolved", 1.0, None, None, 450.0, None, 1.5, None, None, None, None, None, None, None, None)
    # deviation NULL -- must be excluded
    db.insert_fair_value_log("resolved", 2.0, None, None, 450.0, None, None, None, None, None, None, None, None, None, None)
    ...
    db.insert_fair_value_log("unresolved", 3.0, None, None, 450.0, None, 2.0, None, None, None, None, None, None, None, None)
    db.close()
...
def test_fit_calibration_none_when_many_rows_but_too_few_windows(tmp_path):
    ...
    for i in range(600):
        db.insert_fair_value_log(
            "cond0", float(i), None, None, 450.0, None, -2.0, None, None, None, None, None, None, None, None
        )
    db.close()
...
def test_fit_calibration_improves_when_analytic_model_is_biased(tmp_path):
    ...
    # coef_ is now [intercept, dev, dev*log_t, log_t]; with t_remaining
    # fixed in the fixture, the intercept and log_t terms are collinear, so
    # the recovered shift is spread across both
    intercept, _dev, _dev_log_t, log_t = report.coef_
    import math

    effective_intercept = intercept + log_t * math.log(450.0)
    assert effective_intercept < -0.5  # recovers the negative shift
